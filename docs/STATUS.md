...
- ✅ **Team & member management** (`/app/team`): invite teammates by email (signed token, expiring), accept flow (only the invited email, joins + switches into the org), change role, remove member (last-owner protected), revoke invitation. **Per-plan seat limits** (free 1 / medium 3 / commercial unlimited; admin per-org `team_quota_override`) enforced server-side (members + pending invites). **Org switcher** in the nav for users in multiple orgs. Verified by tests.
